  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="text-sm text-gray-500 mb-4">
        ...
        <span className="text-gray-900 font-medium">Sınıf Kayıt</span>
      </div>

      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 mb-1">Sınıf Kayıt Modülü</h1>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <Button
          variant="primary"
          onClick={() => router.push('/registration?type=class&action=add')}
            ...
            <tbody>
              {classes.map((classItem) => (
                <tr key={classItem.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4 text-sm text-gray-900">{classItem.name}</td>
                  <td className="py-3 px-4">
                    <div className="flex gap-2">
                      <button
                        onClick={() => router.push(`/registration?type=class&action=edit&id=${classItem.id}`)}
